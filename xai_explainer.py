...
8. HiRes-CAM: Similar to Grad-CAM but uses gradients without averaging, giving sharper results.
...
# Task 2 
from train_fresh_rotten_classifier import build_model, _extract_checkpoint_state_dict
...
class ProduceXAI:
    ...
    def generate_lime_explanations(self, image_path: str | Path, num_samples = 500) -> Image.Image:
        """
        ...
        return Image.fromarray(img_boundary_uint8)
    ...
    def generate_counterfactual(self, image_path: str | Path) -> Tuple[Image.Image, str]:
        ...
        # We restore by replacing a segment with the average color of the healthy parts of the fruit
        restored_proxy = cv2.GaussianBlur(img_np, (55, 55), 0)

        for i in range(np.max(segments) + 1):
            temp_img = copy.deepcopy(img_np)
            temp_img[segments == i] = restored_proxy[segments == i]
        ...
        # Create a mask where only the dealbreaker is highlighted
        mask = np.zeros(segments.shape, dtype=bool)
        if dealbreaker_segment != -1:
            mask[segments == dealbreaker_segment] = True
        
        # Darken the whole image and then highlight the dealbreaker
        # This shows the producer exactly what needs to change
        overlay = img_np.copy()
        overlay[~mask] = overlay[~mask] * 0.3 # Dim everything else
        
        # Add a colored border around the dealbreaker
        result_img = label2rgb(mask, overlay, colors=[(0, 255, 0)], alpha=0.3, bg_label=0)
        
        explanation_text = (
            f"Current Fresh Confidence: {initial_fresh_score:.2f}\n"
            f"If the highlighted region were improved, 'Fresh' confidence "
            f"would increase by {best_improvement*100:.1f}%."
        )

        return Image.fromarray((result_img * 255).astype(np.uint8)), explanation_text
    ...
    def generate_hires_cam(self, image_path: str | Path) -> Image.Image:
        """
        Generates a HiRes-CAM heatmap. A mathematically more accurate version of Grad-CAM for ResNet models.
        """
        ...
        with HiResCAM(model=ClassifierWrapper(self.model).to(self.device), 
    ...
    def generate_master_audit_report(self, image_path: str | Path) -> Image.Image:
        ...
        m2 = self.generate_hires_cam(image_path)
        m3 = self.generate_score_cam(image_path)
        m4 = self.generate_lime_explanations(image_path)
        ...
        m9, _ = self.generate_counterfactual(image_path)
        m10 = self.generate_smoothgrad(image_path)

        # Create grid list
        images = [original_img, m1, m2, m3, m4, m5, m6, m7, m8, m9, m10]
        titles = [
            "Original Image", "1. Grad-CAM", "2. HiRes-CAM", 
        ...
        plt.clf()
        fig, axes = plt.subplots(3, 4, figsize=(24, 18), facecolor='#F5F5F5')
        fig.suptitle(f"Bristol Food Network: AI Transparency Audit\nProduct: {Path(image_path).name}", 
...
if __name__ == "__main__":
    MODEL_PATH = "fresh_rotten_resnet501.pth" 
    
    # Pick a random image from the dataset for testing
    TEST_IMAGE_PATH = r"FruitAndVegetableDataset\Fruit And Vegetable Diseases Dataset\Apple__Rotten\rotated_by_15_Screen Shot 2018-06-07 at 2.16.54 PM.png"
    
    if Path(MODEL_PATH).exists() and Path(TEST_IMAGE_PATH).exists():
        xai = ProduceXAI(model_path=MODEL_PATH)

        master_report = xai.generate_master_audit_report(TEST_IMAGE_PATH).save("ADMIN_XAI_REPORT.jpg")
