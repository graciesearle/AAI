...
DEFAULT_DATASET_DIR = SCRIPT_DIR.parent / "task2_data" / "Fruit And Vegetable Diseases Dataset"
DEFAULT_MODEL_PATH = SCRIPT_DIR / "fresh_rotten_resnet501.pth"
DEFAULT_PLOT_PATH = SCRIPT_DIR / "learning_curves.png"
DEFAULT_KAGGLE_DOWNLOAD_DIR = SCRIPT_DIR
...
def _load_project_env_files() -> None:
    """Load .env files from the script directory and its parent folders."""
...
def _resolve_dataset_dir(default_path: Path, env_name: str = "TASK2_DATASET_DIR") -> Path:
    """Prefer a valid env override, otherwise fall back to the bundled task2_data path."""
    env_value = os.environ.get(env_name, "").strip()
    if env_value:
        env_path = Path(env_value).expanduser()
        if not env_path.is_absolute():
            env_path = (SCRIPT_DIR.parent / env_path).resolve()
        if env_path.exists():
            return env_path
...
class RunConfig:
    ...
    save_model_path: Path = DEFAULT_MODEL_PATH
    save_plot_path: Path = DEFAULT_PLOT_PATH
    ...
    kaggle_username: str = ""
    kaggle_key: str = ""
    kaggle_download_dir: Path = DEFAULT_KAGGLE_DOWNLOAD_DIR


CONFIG = RunConfig(
    dataset_dir=_resolve_dataset_dir(DEFAULT_DATASET_DIR),
    epochs=_env_int("TASK2_EPOCHS", 20),
    batch_size=_env_int("TASK2_BATCH_SIZE", 32),
    learning_rate=_env_float("TASK2_LEARNING_RATE", 1e-3),
    quality_loss_weight=_env_float("TASK2_QUALITY_LOSS_WEIGHT", 0.4),
    image_size=_env_int("TASK2_IMAGE_SIZE", 224),
    patience=_env_int("TASK2_PATIENCE", 5),
    num_workers=_env_int("TASK2_NUM_WORKERS", 2),
    seed=_env_int("TASK2_SEED", 42),
    no_pretrained=not _env_bool("TASK2_USE_PRETRAINED", True),
    save_model_path=Path(
        _env_str("TASK2_SAVE_MODEL_PATH", str(DEFAULT_MODEL_PATH))
    ).expanduser(),
    save_plot_path=Path(
        _env_str("TASK2_SAVE_PLOT_PATH", str(DEFAULT_PLOT_PATH))
    ).expanduser(),
    predict_image=(
        Path(_env_str("TASK2_PREDICT_IMAGE", "")).expanduser()
        if _env_str("TASK2_PREDICT_IMAGE", "")
        else None
    ),
    auto_download_from_kaggle=_env_bool("TASK2_ALLOW_KAGGLE_DOWNLOAD", False),
    kaggle_dataset=_env_str(
        "TASK2_KAGGLE_DATASET",
        "muhammad0subhan/fruit-and-vegetable-disease-healthy-vs-rotten",
    ),
    kaggle_username=_env_str("KAGGLE_USERNAME", ""),
    kaggle_key=_env_str("KAGGLE_KEY", ""),
    kaggle_download_dir=Path(
        _env_str("TASK2_KAGGLE_DOWNLOAD_DIR", str(DEFAULT_KAGGLE_DOWNLOAD_DIR))
    ).expanduser(),
)
...
def _validate_kaggle_placeholders(cfg: RunConfig) -> None:
    ...
    if not cfg.kaggle_username.strip() or cfg.kaggle_username.strip().startswith("YOUR_"):
        raise ValueError(
            "Set KAGGLE_USERNAME (or CONFIG.kaggle_username) before enabling Kaggle downloads."
        )

    if not cfg.kaggle_key.strip() or cfg.kaggle_key.strip().startswith("YOUR_"):
        raise ValueError(
            "Set KAGGLE_KEY (or CONFIG.kaggle_key) before enabling Kaggle downloads."
        )
...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        progress_interval = max(1, num_train_batches // 10)

        for batch_index, (images, labels, quality_targets) in enumerate(train_loader, start=1):
            images = images.to(device)
            labels = labels.to(device)
            quality_targets = quality_targets.to(device)
            ...
            if batch_index % progress_interval == 0 or batch_index == num_train_batches:
                elapsed = time.perf_counter() - epoch_start
                avg_batch_seconds = elapsed / max(batch_index, 1)
                remaining_batches = num_train_batches - batch_index
                eta_seconds = avg_batch_seconds * max(remaining_batches, 0)
                progress_pct = 100.0 * batch_index / max(num_train_batches, 1)
                print(
                    f"  Epoch {epoch + 1}/{num_epochs} progress: "
                    f"{batch_index}/{num_train_batches} batches ({progress_pct:.1f}%) "
        ...
        print(
            ...
            f"Epoch Time: {time.perf_counter() - epoch_start:.1f}s"
        )
...
def main() -> None:
    ...
    force_train = _env_bool("TASK2_FORCE_TRAIN", False)
    trained_now = False
    history = History(
        train_total_loss=[],
        val_total_loss=[],
    ...
    if cfg.save_model_path.exists() and not force_train:
        try:
            checkpoint = torch.load(cfg.save_model_path, map_location=device)
            model.load_state_dict(checkpoint["model_state_dict"])
            print(
                f"Loaded existing model from: {cfg.save_model_path} "
                "(set TASK2_FORCE_TRAIN=1 to retrain)."
            )
        except (KeyError, RuntimeError, OSError) as exc:
            print(
                "Existing model checkpoint could not be loaded cleanly "
                f"({exc}). Starting fresh training run."
            )
        ...
        if force_train:
            print("TASK2_FORCE_TRAIN=1 detected. Training from scratch.")
        else:
    ...
    run_full_eval_on_load = _env_bool("TASK2_FULL_EVAL_ON_LOAD", False)
    if trained_now or run_full_eval_on_load:
        ...
        print(
            "Skipped full train/validation evaluation because training was skipped "
            "(set TASK2_FULL_EVAL_ON_LOAD=1 to enable)."
        )
    ...
    if trained_now:
        ...
        print("Skipped saving and plotting because model training was skipped.")
